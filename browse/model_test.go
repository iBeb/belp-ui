...
// q quits from the list, the way every other full-screen program does — and types
// itself in the search field, where it is a letter. ^Q cannot be relied on: a
// multiplexer binds it to quit the session and takes it first.
func TestQuitsOnQFromTheListOnly(t *testing.T) {
	m := model(10)
	if m.Focus() != FocusList {
		t.Fatalf("Focus() = %v, want the list to start with", m.Focus())
	}
	_, cmd := m.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("q")})
	if cmd == nil {
		t.Fatal("q on the list did nothing")
	}
	if _, ok := cmd().(QuitMsg); !ok {
		t.Errorf("q on the list sent %T, want QuitMsg", cmd())
	}

	// In the search field it is a letter like any other.
	m.chrome.Focus = FocusSearch
	typed, cmd := m.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("q")})
	if cmd != nil {
		if _, ok := cmd().(QuitMsg); ok {
			t.Error("q quit from the search field, where it is a letter")
		}
	}
	if before, after := m.Query(), typed.Query(); after != before+"q" {
		t.Errorf("Query() = %q, want %q with the letter typed", after, before)
	}

	// And the filter bar, where it is neither.
	m.chrome.Focus = FocusFilters
	bar, cmd := m.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("q")})
	if cmd != nil {
		if _, ok := cmd().(QuitMsg); ok {
			t.Error("q quit from the filter bar")
		}
	}
	if before, after := m.Query(), bar.Query(); after != before {
		t.Errorf("Query() = %q, want it left at %q from the filter bar", after, before)
	}
}
